...
public class FichierController {
    ...
    public FichierController(StockageService stockageService, FichierRepository fichierRepository, CompteRepository compteRepository) {
    ...
    public ResponseEntity<?> sendFichier(@PathVariable("src_id") Integer src, @RequestParam("file") MultipartFile fichier, @PathVariable("dest_id") Integer dest) throws IOException {
        Compte source = compteRepository.findById(src).orElseThrow(() -> new CompteNotFoundException(src));
        Compte destination = compteRepository.findById(dest).orElseThrow(() -> new CompteNotFoundException(dest));
